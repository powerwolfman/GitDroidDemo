...
import com.lifucong.gitdroiddemo.login.TokenInterceptor;
...
public class GithubClient implements GithubApi{
    ...
    private GithubClient(){
        ...
        Retrofit retrofit = new Retrofit.Builder()
                ...
                /**
                 * Retrofit 强大的功能：Gson转换器----将我们的数据请求的结果进行json转换，
                 * 转换为我们需要的类型,例如类或者集合
                 */
    ...
    @Override
    public Call<RepoResult> searchRepos(@Query("q") String query,
                                        @Query("page") int pageId) {
        return githubApi.searchRepos(query, pageId);
    }

    @Override
    public Call<RepoContentResult> getReadme(@Path("owner") String owner,
